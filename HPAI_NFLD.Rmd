...
#Summary Stats
```{r Summary stats}
whole_dataset <- read.csv("data/2023-06-22_HPAI_Mortality_Tracker - Newfoundland.csv", na = "NA")

#Total mortality
sum(hpai$Total)
sum(whole_dataset$TotalObserved)

#Total mortality by species
summaryBy(TotalObserved ~ CommonName, data = whole_dataset, FUN = "sum")
...
#Static Maps
```{r Species Maps}
species_map_data <- summaryBy(Total ~ Lat + Long + Species, data = top4, FUN = sum) %>%
  mutate(TotalCat = ifelse(Total.sum > 50, "> 50", ifelse(Total.sum < 10, "< 10", "10 to 50")))
...
ggplot() +
  ...
  geom_point(data = species_map_data %>% filter(Long < 0), aes(y = Lat, x = Long, colour = TotalCat, shape = Species)) +
...
#Puffins and Kittiwakes do not have an observation >50 so it changes the colours - make sure colours are the same
show_col(viridis_pal(option = "D")(6))
cols <- c("< 10" = "#440154FF", "10 to 50" = "#21908CFF", "> 50" = "#7AD151FF")
...
a <- ggplot() +
  ...
  scale_colour_manual(name = "Total", values = cols)+
  geom_point(data = species_map_data %>% filter(Species == "NOGA"), aes(y = Lat, x = Long, colour = TotalCat), size = 1.5) +
  guides(colour = guide_legend(title = "Number of Carcasses"))+
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), title = element_text( size = 10, face = "bold"), legend.position = "none", axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10))
a
...
b <- ggplot() +
  ...
  geom_point(data = species_map_data %>% filter(Species == "COMU"), aes(y = Lat, x = Long, colour = TotalCat), size = 1.5) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), title = element_text( size = 10, face = "bold"), legend.position = "none", axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10))
b
...
c <- ggplot() +
  ...
  geom_polygon(data = Canada, aes(x = long, y = lat, group = group), colour = "black", fill = "lightgrey") +
  scale_colour_manual(values = cols)+
  geom_point(data = species_map_data %>% filter(Species == "ATPU"), aes(y = Lat, x = Long, colour = TotalCat), size = 1.5) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), title = element_text( size = 10, face = "bold"), legend.position = "none", axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10))
c
...
d <- ggplot() +
  ...
  geom_polygon(data = Canada, aes(x = long, y = lat, group = group), colour = "black", fill = "lightgrey") +
  scale_colour_manual(values = cols)+
  geom_point(data = species_map_data %>% filter(Species == "BLKI"), aes(y = Lat, x = Long, colour = TotalCat), size = 1.5) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), title = element_text( size = 10, face = "bold"), legend.position = "none", axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10))
d

jpeg(filename = "figures/Species_map.jpeg", width = 18, height = 20, units = "cm", res = 300)
a + b + d + c
dev.off()
```
...
e <- ggplot() +
  ...
  geom_polygon(data = Canada, aes(x = long, y = lat, group = group), colour = "black", fill = "lightgrey") +
  scale_colour_manual(values = cols)+
  geom_point(data = top4 %>% filter(Month < 7), aes(y = Lat, x = Long, colour = TotalCat))+
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), title = element_text( size = 10, face = "bold"), axis.text.x = element_text(size = 12), legend.position = "none", axis.text.y = element_text(size = 12))
...
g <- ggplot() +
  ...
  geom_polygon(data = Canada, aes(x = long, y = lat, group = group), colour = "black", fill = "lightgrey") +
  scale_colour_manual(values = cols)+
  geom_point(data = top4 %>% filter(Month == 8), aes(y = Lat, x = Long, colour = TotalCat))+
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), title = element_text( size = 10, face = "bold"), legend.position = "none", axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12))
...
h <- ggplot() +
  ...
  geom_polygon(data = Canada, aes(x = long, y = lat, group = group), colour = "black", fill = "lightgrey") +
  scale_colour_manual(values = cols)+
  geom_point(data = top4 %>% filter(Month == 9), aes(y = Lat, x = Long, colour = TotalCat))+
  theme(panel.background = element_rect(fill = 'white', colour = 'black'), title = element_text( size = 10, face = "bold"), legend.position = "none", axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12))

jpeg(filename = "figures/Monthly_map.jpeg", width = 18, height = 20, units = "cm", res = 300)
e+f+g+h
dev.off()
```
...
sub_id2 = subset(gif_data, Date <= "2022-07-12")
...
#Peak Mortality
``` {r Time series plot}
freqplot_all <- summaryBy(Total ~ Date, data = hpai, FUN = c(sum))
...
ggplot(data = freqplot_all, aes(y = Total.sum, x = Date)) +
  geom_segment(aes(x = as.Date("2022-07-15"), y = 100, xend = as.Date("2022-08-19"), yend = 100), colour = 'lightgrey', size = 1000) +
  geom_point() +
